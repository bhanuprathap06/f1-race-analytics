...
def engineer_features(driver_name, year, total_points, circuit):
    ...
    # Driver skill (0-1 scale)
    driver_skill = min(1.0, driver_avg_champ_pts / 150.0)
    ...
    features = {
        ...
        'driver_points_std': max(0.5, avg_pts * (0.4 - 0.2 * driver_skill)),
        'driver_avg_position': max(1, 22 - (avg_pts * 2.5 * performance_ratio)),
        'driver_best_position': max(1, int(1 + (20 - 20 * driver_skill))),
        'driver_worst_position': min(20, int(18 + (2 * (1 - driver_skill)))),
        'driver_position_std': max(1, 6 * (1 - driver_skill)),
        ...
        'grid_to_position_diff': 2 * (1 - driver_skill),
